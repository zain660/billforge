...
@section('content')
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('subscriptions.admin.packages.update', $package) }}" method="POST"
                ...
                <div class="md:w-2/3 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-4 py-2.5 outline-none transition shadow-sm">{{ old('description', $package->description) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="currency" required
                                class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-4 py-2.5 outline-none transition shadow-sm">
                                <option value="USD" {{ old('currency', $package->currency) == 'USD' ? 'selected' : '' }}>
                                    USD</option>
                                <option value="EUR" {{ old('currency', $package->currency) == 'EUR' ? 'selected' : '' }}>
                                    EUR</option>
                                <option value="GBP" {{ old('currency', $package->currency) == 'GBP' ? 'selected' : '' }}>
                                    GBP</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="billing_cycle" required
                                ...
                                <option value="monthly"
                                    {{ old('billing_cycle', $package->billing_cycle) == 'monthly' ? 'selected' : '' }}>
                                    Monthly
                                </option>
                                <option value="yearly"
                                    {{ old('billing_cycle', $package->billing_cycle) == 'yearly' ? 'selected' : '' }}>Yearly
                                </option>
                                <option value="lifetime"
                                    {{ old('billing_cycle', $package->billing_cycle) == 'lifetime' ? 'selected' : '' }}>
                                    Lifetime
                                    (One-time)</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="number" name="trial_days"
                                value="{{ old('trial_days', $package->trial_days ?? 0) }}" required
                                class="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-4 py-2.5 outline-none transition shadow-sm"
                                placeholder="e.g. 14">
                        </div>
                    </div>
            ...
            <div class="p-8 border-b border-gray-100 flex flex-col md:flex-row gap-8 bg-slate-50">
                ...
                <div class="md:w-2/3 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="paypal_plan_id"
                            value="{{ old('paypal_plan_id', $package->paypal_plan_id) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="authorize_plan_id"
                            value="{{ old('authorize_plan_id', $package->authorize_plan_id) }}"
            ...
            <div class="p-8 flex flex-col md:flex-row gap-8 border-b border-gray-100">
                ...
                <div class="md:w-2/3">
                    ...
                    {{-- Existing features for JS --}}
                    <script id="existing-features-data" type="application/json">
                    @json($package->routes->map(fn($r) => ['name' => $r->feature_name, 'route' => $r->route_name]))
                </script>
            ...
            <div class="bg-gray-50 p-6 flex items-center justify-end gap-3">
                ...
                <button type="submit"
                    ...
                    <i class="fa-solid fa-save mr-2"></i> Update Package
                </button>
            </div>
        </form>
    </div>
    ...
    <script>
        (function() {
            ...
            const routes = JSON.parse(document.getElementById('route-options-data').textContent);
            const existingFeatures = JSON.parse(document.getElementById('existing-features-data').textContent);
            ...
            // Pre-populate from old() on validation fail, otherwise from existing
            @if (old('features'))
            ...
            @else
                if (existingFeatures.length > 0) {
                    existingFeatures.forEach(f => addFeatureRow(f.name || '', f.route || ''));
                } else {
                    addFeatureRow();
                }
            @endif
